...
public class ArrBag<T>
{
    ...
    public ArrBag<T> union( ArrBag<T> other )
	{
		...
		//for every elem in this array 
		//search contain element in ur array , result set
		...
		//take from u array and put into result
		//other set if not in result set 
	...
	{
		ArrBag<T> intersection = new ArrBag<T>(other.size() ); // REPLACE WITH YOUR CODE
		// YOUR LOOPS HERE
		for ( int i=0 ; i < other.size() ; ++i ){
